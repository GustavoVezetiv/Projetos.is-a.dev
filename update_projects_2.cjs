...
const startIdx = content.indexOf('<div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">');
const endStr = '</div>\n              </div>\n              \n         \n          </React.Fragment>\n      );\n  }';
const endIdx = content.indexOf(endStr);
const endIdxAlt = content.indexOf('</React.Fragment>', startIdx);

if (startIdx !== -1 && endIdxAlt !== -1) {
    // Find the closing div of the grid. It's right before </div></div></React.Fragment>
    // We can just use string slice
    const beforeGrid = content.slice(0, startIdx + '<div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">'.length);
    const afterGrid = content.slice(endIdxAlt - '</div>\n              </div>\n              \n         \n          '.length);
    
    // Actually, an easier way is to use regex replacing the contents of the grid container
    let updatedContent = content.substring(0, startIdx + '<div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">'.length) + 
                         '\n' + newProjetosContent + 
                         '                  </div>\n              </div>\n              \n         \n          </React.Fragment>';
                         
    // Re-attach everything after </React.Fragment>
    updatedContent += content.substring(endIdxAlt + '</React.Fragment>'.length);
